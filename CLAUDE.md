...
- **Obstacles, two kinds**:
  - `solidObstacles` — rocks, gate posts, tire stacks, street lamp poles. Circle collision that pushes the car out and reverses speed. Positions are **cached in the array at creation**; moving a solid's mesh later will not move its collision.
...
- **Camera**: fixed-offset follow (`CAMERA_OFFSET`) with frame-rate-independent damping (`1 - Math.exp(-k*dt)`), plus mouse parallax. It never rotates with the car. The goal points add `velocity / CAMERA_DAMP` feed-forward — this cancels the lerp's steady-state lag so the car stays screen-centered at speed; remove it and the car drifts toward the screen edge when driving.
- **Night lighting**: the car has a forward SpotLight headlamp; four street lamps around the track are emissive poles with warm PointLights plus additive pool discs. The floor and track asphalt are deliberately `MeshStandardMaterial` (not Basic) so these lights show on them — switching them back to unlit materials makes headlights/street lights invisible on the ground.
...
- **Input**: keyboard map (WASD + arrows; arrows call `preventDefault` so they don't scroll) and a virtual joystick that is only *displayed* on touch devices via `@media (hover: none) and (pointer: coarse)` — its JS handlers are always attached. The joystick is direction-based, not throttle/steer: the stick points where the car should go on screen (camera never rotates, so stick up = world −z), and pushing roughly opposite the car's nose reverses instead of turning around.
